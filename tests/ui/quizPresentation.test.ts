...
describe('quiz presentation helpers', () => {
  it('builds mobile-ready progress labels for incomplete quizzes', () => {
    ...
    expect(summary.modeLabel).toBe('Instinct mode');
    expect(summary.keyboardHint).toContain('A/B/C/D');
  });
  ...
  it('switches labels when the quiz is complete', () => {
    ...
    expect(summary.modeLabel).toBe('Review before result');
  ...
  it('builds review dot labels, accessibility text, and classes', () => {
    ...
    expect(dots[0]).toMatchObject({ label: '1A', isAnswered: true, isCurrent: false });
    expect(dots[1]).toMatchObject({ label: '2B', isAnswered: true, isCurrent: true });
