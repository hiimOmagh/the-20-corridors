...
describe('public result route skeleton policy', () => {
  it('defines only the approved public-result API route files', () => {
    ...
    expect(PUBLIC_RESULT_ROUTE_SKELETON_POLICY.mode).toBe('approved-route-files-dry-run-handlers-only');
  });
  ...
  it('keeps planned methods aligned with the API DTO contract', () => {
    ...
    expect(PUBLIC_RESULT_ROUTE_SKELETON_POLICY.allowedMethods).toEqual(['POST', 'GET', 'DELETE']);
  });
  ...
  it('detects blocked private transport and production implementation text', () => {
    expect(containsBlockedPublicResultRouteSkeletonText('export async function POST() { return Response.json({}); }')).toBe(false);
    expect(containsBlockedPublicResultRouteSkeletonText('const rawAnswers = []')).toBe(true);
    expect(containsBlockedPublicResultRouteSkeletonText('const prisma = new PrismaClient()')).toBe(true);
    expect(containsBlockedPublicResultRouteSkeletonText('const planned = true')).toBe(false);
  });
});
