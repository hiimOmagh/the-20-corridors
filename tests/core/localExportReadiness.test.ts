...
describe('local export readiness contract', () => {
  it('passes the Phase 4.2 hardened local export gate', () => {
  ...
  it('records export boundary docs, scripts, and allowed local surfaces', () => {
    expect(report.schemaVersion).toBe('phase-4.2-local-export-readiness-v1');
    expect(report.metadata.exportMode).toBe('local-share-card-image-export-hardened');
    ...
    expect(report.surfaces.allowedLocalExportSurfaces).toContain('hardened-export-status-and-failure-copy');
    ...
    expect(report.coverage.approvedImageExportSignalCount).toBeGreaterThanOrEqual(7);
  });
  ...
  it('keeps the export surface free from raw answers, full-result serialization, and blocked scope', () => {
    ...
    expect(report.privacy.approvedImageExportSignals.length).toBeGreaterThanOrEqual(7);
  });
...
function makeMinimalTempRepoRoot(): string {
  ...
  writeFileSync(path.join(tempRoot, 'docs/ui/phase-4-2-export-ux-hardening-failure-state-polish-status.md'), '# Status\n');
  writeFileSync(
  ...
  writeFileSync(
    path.join(tempRoot, 'src/features/results/resultShareImageExport.ts'),
    'import type { LocalShareCardPreview } from "./resultShareCard"; export const LOCAL_SHARE_IMAGE_EXPORT_SCHEMA_VERSION = "test"; export function buildLocalShareImageExportUxDetails(){} export function getLocalShareImageExportCapability(){} export function buildLocalShareCardExportSvg(card: LocalShareCardPreview){ return card.title; } export function exportLocalShareCardPng(){ const canvas = document.createElement("canvas"); canvas.toBlob(() => null); URL.createObjectURL(new Blob()); const anchor = document.createElement("a"); anchor.download = "test.png"; }\n'
  );
