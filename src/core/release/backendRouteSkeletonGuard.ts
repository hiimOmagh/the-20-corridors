...
export const BACKEND_ROUTE_SKELETON_GUARD_SCHEMA_VERSION = 'phase-7.3-backend-route-skeleton-guard-v2' as const;
...
export interface BackendRouteSkeletonGuardReport {
  ...
  readonly metadata: {
    ...
    readonly phaseScope: 'phase-7-approved-route-files-dry-run-handlers';
  ...
  readonly gates: {
    ...
    readonly approvedRouteFilesPresent: boolean;
    readonly onlyApprovedRouteFilesPresent: boolean;
    readonly onlyApprovedRequestHandlersPresent: boolean;
    readonly routeFilesUseDryRunHandlersOnly: boolean;
  ...
  readonly implementationScan: {
    ...
    readonly unapprovedRouteFiles: readonly string[];
...
export async function runBackendRouteSkeletonGuard(
  ...
  const allowedRouteFiles = listPublicResultRouteSkeletonFiles();
  const actualRouteFiles = existingPaths(repoRoot, allowedRouteFiles);
  const routeSource = actualRouteFiles.map((file) => readOptionalFile(repoRoot, file)).join('\n');
  const unapprovedRouteFiles = findUnapprovedPublicResultApiRouteFiles(repoRoot, allowedRouteFiles);
  ...
  const requestHandlerSignals = findSignals(routeSource, REQUEST_HANDLER_SIGNALS);
  const blockedImplementationSignals = findSignals(routeSource, BLOCKED_IMPLEMENTATION_SIGNALS);
  const rawOrFullResultSignals = findSignals(routeSource, RAW_OR_FULL_RESULT_SIGNALS);
  ...
  const gates = {
    ...
    plannedRouteFilesDefined:
      ...
      allowedRouteFiles.every((file) => file.startsWith('src/app/api/public-results')),
    plannedMethodsMatchApiContract: plannedMethods.join('|') === ['POST', 'GET', 'DELETE'].join('|'),
    requestHandlingBoundaryDefined:
      PUBLIC_RESULT_ROUTE_SKELETON_REQUEST_HANDLING_BOUNDARY.length >= 7 &&
      PUBLIC_RESULT_ROUTE_SKELETON_BLOCKED_BEHAVIOR.length >= 9,
    futureGuardExpectationsDefined: PUBLIC_RESULT_ROUTE_SKELETON_REQUIRED_FUTURE_GUARDS.length >= 8,
    approvedRouteFilesPresent: actualRouteFiles.length === allowedRouteFiles.length,
    onlyApprovedRouteFilesPresent: unapprovedRouteFiles.length === 0,
    onlyApprovedRequestHandlersPresent:
      requestHandlerSignals.join('|') === ['export async function POST', 'export async function GET', 'export async function DELETE'].join('|'),
    routeFilesUseDryRunHandlersOnly:
      routeSource.includes('handlePublicResultCreateRouteBody') &&
      routeSource.includes('handlePublicResultReadRoute') &&
      routeSource.includes('handlePublicResultDeleteRouteBody') &&
      !containsBlockedPublicResultRouteSkeletonText(routeSource),
  ...
  return {
    ...
    metadata: {
      ...
      phaseScope: 'phase-7-approved-route-files-dry-run-handlers',
    ...
    routeSkeleton: {
      plannedRouteFiles: allowedRouteFiles,
    ...
    coverage: {
      ...
      plannedRouteFileCount: allowedRouteFiles.length,
...
function buildIssues(
  ...
  if (!gates.backendApiBoundaryPassed) issues.push('backend_api_boundary_contract_failed');
  if (!gates.routeSkeletonGuardScriptExists) issues.push('missing_backend_route_skeleton_guard_script');
  ...
  if (!gates.routeSkeletonDocExists) issues.push('missing_phase7_1_route_skeleton_doc');
  if (!gates.phase71StatusDocExists) issues.push('missing_phase7_1_status_doc');
  ...
  if (!gates.requestHandlingBoundaryDefined) issues.push('request_handling_boundary_incomplete');
  if (!gates.futureGuardExpectationsDefined) issues.push('future_guard_expectations_incomplete');
  if (!gates.approvedRouteFilesPresent) issues.push('approved_route_files_missing');
  if (!gates.onlyApprovedRouteFilesPresent) issues.push('unapproved_public_result_route_files_detected');
  if (!gates.onlyApprovedRequestHandlersPresent) issues.push('route_handlers_not_limited_to_post_get_delete');
  if (!gates.routeFilesUseDryRunHandlersOnly) issues.push('route_files_not_wired_to_dry_run_handlers');
...
function findUnapprovedPublicResultApiRouteFiles(repoRoot: string, allowedRouteFiles: readonly string[]): readonly string[] {
  const routeRoot = path.join(repoRoot, 'src/app/api/public-results');
  if (!existsSync(routeRoot)) return [];
  const allowed = new Set(allowedRouteFiles);
  return listRouteFiles(routeRoot)
...
function listRouteFiles(root: string): string[] {
  ...
  for (const entry of readdirSync(root).sort()) {
    ...
    if (stat.isDirectory()) files.push(...listRouteFiles(absolute));
